/**
 * Carries the old portal's files into the file store, as `carry.ts` plans.
 *
 * Run beside the store, with the old directories mounted read-only:
 *
 * - `HUMANDBS_CARRY_SOURCE` — the directory the old portal served `/files/`
 *   from (default `/source/files`)
 * - `HUMANDBS_CARRY_ASSETS` — its article assets, `public-files` (default
 *   `/source/public-files`)
 * - `HUMANDBS_CARRY_CENSUS` — the census of the first (default
 *   `input/l12/file-census.json`)
 *
 * **It can be stopped and run again.** A file already in the store at the same
 * size is left as it is, so a second run carries only what the first did not
...
import { MULTIPART_CONCURRENCY, MULTIPART_PART_SIZE, MULTIPART_THRESHOLD, PRIVATE_BUCKET } from "~/files/box"
import { contentTypeOf } from "~/files/content-types"

import { planAssets, planCarry, type Carry, type Census } from "./carry"

const INPUT = join(import.meta.dirname, "input", "l12")
const SOURCE = process.env.HUMANDBS_CARRY_SOURCE ?? "/source/files"
const ASSETS = process.env.HUMANDBS_CARRY_ASSETS ?? "/source/public-files"
...
async function storedSize(one: Carry): Promise<number | null> {
  try {
...
async function put(path: string, one: Carry, size: number): Promise<void> {
...
const census = JSON.parse(readFileSync(process.env.HUMANDBS_CARRY_CENSUS ?? join(INPUT, "file-census.json"), "utf8")) as Census
const ids = await researchIds()
const plan = planCarry(census, (hum) => ids.get(hum))
...
const items = [
  ...
  ...plan.carry.map((one) => ({ one, path: join(SOURCE, one.source) })),
]

const total = items.reduce((sum, { one }) => sum + one.size, 0)
const droppedBytes = plan.dropped.reduce((sum, one) => sum + one.size, 0)
console.log(`carry ${items.length} files, ${total} B (assets ${assets.length}); leave ${plan.dropped.length} files, ${droppedBytes} B`)
...
const report = { carried: [] as string[], skipped: [] as string[], missing: [] as string[], resized: [] as string[], failed: [] as string[] }
if (!DRY_RUN) {
  let done = 0
  let bytes = 0
  ...
  writeFileSync(join(INPUT, "out", "carry-report.json"), `${JSON.stringify({ ...report, stray }, null, 2)}\n`)
  console.log(`carried ${report.carried.length}, already there ${report.skipped.length}, missing ${report.missing.length}, `
