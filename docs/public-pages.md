...
本文性のある値 (研究概要・リリースノート・記述系のキー) は markdown で、**server 側で HTML にして
渡す。** 解析器がブラウザに届かないのと、公開後に変わらないテキストを読者ごとに組み直さないため。

**生の HTML は捨てずに通し、許可リストで濾す。** 記事の本文には `<br>` と `<sup>` が実在し、markdown に
対応する構文が無いので移行でも text 側に残す。content はポータルの origin で描画され、移行と提供者からも
入ってくるので、濾さない選択肢は無い。
