...
- `compose.deploy.yml` の `userns_mode` (`keep-id`) は、`db`・`s3`・`tools` の container の中のユーザーを、配置先のユーザーに対応させる設定である。この設定が無いと、bind した dir に書けないか、持ち主の分からないファイルが残る。rootless podman では、container の中の root 以外のユーザーがホストでは別の uid になるためである。保存先が共有のファイルシステムのときに特に問題になる。
