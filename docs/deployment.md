...
- 移行を実行し直しても、搬入は実行し直さなくてよい。非公開のファイルは研究の実体ごとの研究のフォルダに置いており、移行は研究と dataset の実体の ID を hum ラベルと dataset の ID から毎回同じ値で作るので、フォルダがずれない。どの研究にも属さない非公開のフォルダに残ったファイルは `out/carry-report.json` の `stray` に並ぶ。
