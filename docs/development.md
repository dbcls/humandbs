...
```bash
docker compose down -v                 # DB・ファイルストア・node_modules の volume ごと消す
...
初回の手順 (`icd10:import` を含む) のあとに実行する。`icd10:import` が `migration/input/` に残した ICD10 の配布物が無いと、読み込みは止まる。

```bash
docker compose exec app npm run db:load-dev-data   # 開発用データを入れる
docker compose exec app npm run s3:common-assets   # 記事が参照する画像と PDF をアップロードする
