...
語彙・数値・単一値と、experiment の表示ラベル・研究題目・提供者名・機関名・研究課題名・研究費の名称 —
は書式を持たず、リンクも単位も語彙のラベルも型から導出して描画側が作る。文は型が `text` のキーと、
...
ここで 1 つに解決する — NHA ID は content の `releaseDate` を、外部 accession はアーカイブの値を使うので、
**どちらが効くかを下流のどこにも判断させない**。
...
**入れる範囲は opt-out で決める。** 公開表現の値は原則すべて連結し、外すのは identity
(uuid・配列要素の id・語彙値の id) と**リンクの行き先**だけ。URL を外すのは、それが画面に文字として
出る値ではないため — URL がそのまま表示文字列になっているものはリンクの文字列の側が残るので、
ドメインで引く経路は消えない。逆に hum ラベルと dataset id は content に無いので pin 台帳から足す。
...
**dataset の日付はここで 1 つに解決する。** ポータルが発行した id は content の値、外部 accession は
アーカイブのキャッシュ (「日付」)。行の側で解決してあるので dataset のページと一覧と JSON API が
食い違えず、キャッシュの更新は行の作り直しで全部に届く。
