...
- **外部アーカイブの dataset には持たせない。** distribution はアーカイブから取る経路のまま。この区別は
  **primary の dataset id の綴り**が決める (「identity と label」) ので、外部の dataset には編集画面が
  picker を出さず、ファイル選択を伴う保存を 400 で弾く
...
- **指し先は NOT NULL で、指されている document は消せない。** 「最新を指す行があるのに指していない」
  状態を作らない
...
- **版を初めて切る操作は「移す」。** いまの本文を `{slug}/version/1` へ rename し、空いた slug で指し先の
  行を作る。本文のコピーが生まれない
