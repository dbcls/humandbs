...
**全文に入れる範囲は opt-out で決める。** content の値は原則すべて入れ、外すのは identity
(uuid・配列要素の id・語彙値の id) だけ。入れると、その文字列が本文に一度も出てこない行が検索に
ヒットする。逆に hum ラベルと dataset id は content に無いので別に足す — 検索窓に最初に打たれるのが
これなので、pin 台帳の側から持ってくる。
