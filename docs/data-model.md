...
内部の同一性と、外に見せる名前を混ぜない。research と dataset と experiment は抽象的な identity を
持ち、**hum ラベル・公開版番号・dataset id はそこに後から pin する label**。label が無くても
オブジェクトを作れ、label が変わっても参照が壊れない。

**pin 台帳が 3 系統のラベルの一意性を 1 箇所で持つ。** 「どのラベルがどの identity に付いていて、
いま公開されているか」を台帳が持ち、取り下げは pin を台帳に残したまま公開状態を落とすこと、
再公開は同じ pin を戻すことになる。これで取り下げと再公開に専用の状態が要らない。

守る不変条件:

- **可視性は公開状態にある版の集合を台帳から引いて判定し、版番号の大小比較で行わない。**
  番号に gap があると `v1..latest` の列挙は未公開の版を素通しする
...
secondary を合わせて台帳で一意で、上の規則はどちらにも同じく適用される。旧 id への外部参照を
解決し続けるための仕組み。
...
```
Research (identity)                    <- hum label pinned here
|
...
|     +-- change entries                  copy-on-write for touched dataset / experiment
|     +-- new dataset identities          enter the ledger on publish
|
+-- Dataset (identity)                 <- dataset id pinned here; belongs to one research
      +-- Experiment (identity)        <- ordered; part of the dataset's content
```
...
**experiment は dataset に従属し、関係は 1 dataset : n experiment。** dataset ごとに違う値は experiment の
値スロットが持つので、どの dataset の experiment かは従属関係そのものが表す。experiment に版は持たせない。
...
| 判定 | 根拠 |
|---|---|
| dataset が公開されているか | published content があるか |
...
**どの公開版からも参照されなくなった dataset (孤児) は削除しない。** 実体は残るが検索用の行に載らない
ので公開ページには出ず、管理画面から復旧できる。research 自体を削除したときだけ、composition に従って
配下の dataset も消える。
...
field は 3 種に分かれ、**分類は値の型が持つ (schema が SSOT)。** 一覧としてこの doc に持たない。
...
experiment の値スロットのキーは、統制語彙の安定した id を identity とする。catalog が表示ラベル
(ja/en)・値の型・表示順を持つ。
...
公開検索と facet が読む行を Postgres の中に持つ。**編集用の行から導出したもので、公開分しか持たない。**

**何が公開されているかの判定をここ 1 つに閉じる。** 公開画面・public API・公開検索は、対象の集合をこの
行からしか引かない。本文そのものは ContentSnapshot と dataset の published content から取るが、これらは
**公開時にしか行が生まれない**テーブルで、編集中の値は draft 配下の変更エントリに入る。編集用と公開用が
同じ DB に同居する以上、担保は物理的な分離ではなくこの 2 点で作る。
...
| bucket | key | 読み手 |
|---|---|---|
| 非公開 | `{researchId}/{ファイル名}` | admin のみ。server function が presigned URL で渡す |
...
0 個以上選ぶ。これが「この dataset のデータはこれ」の注記になる。
