...
(uuid・配列要素の id・語彙値の id)、**リンクの行き先**、そして **dataset のファイル選択**。URL を外すのは、
それが画面に文字として出る値ではないため — URL がそのまま表示文字列になっているものはリンクの文字列の
側が残るので、ドメインで引く経路は消えない。逆に hum ラベルと dataset id は content に無いので pin
台帳から足す。
