...
| 誰が | 何を |
|---|---|
| admin | 自由文キーの追加・編集・削除・並び替え、facet のラベル・カテゴリ・並び、公開ページに出すかどうか、ポータル由来の語彙値の追加・rename・非活性化 |
| 開発 | キーの型を変えること、外部標準の語彙の取り込みと更新、語彙値の併合 |
...
- **使われているものは消せない。** キーは公開・draft のどちらかの content が値を持てば使用中で、
  ...
  指しているデータからは引ける
- **外部標準の語彙値は読み取り専用。** 取り込みで丸ごと入れ替わるので、ここでの手修正は次の取り込みで
  痕跡も残さずに消える
...
配布物が章の名前を持たず、絞り込みに要るのは 3 桁と 4 桁の関係だけ。**語彙値は出所を持ち、外部標準
由来は読み取り専用**。出所を持たないと、手修正が次の取り込みで黙って消える。
