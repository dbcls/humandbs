...
**dataset は箱の list に対する選択を持てる。** 自分が属する hum の箱の list に現れるノードを 0 個以上
選ぶ。これが「この dataset のデータはこれ」の注記になり、dataset content の中に入る。

- **選択は集合で、順序を持たない。** ページは dataset のファイルを箱の list の順 (名前順) で並べるので、
  選んだ順は誰にも見えない。content には名前順で書き、同じファイルを別の順で選んだ 2 つは同じ選択として
  比べる (`app/files/selection.ts`)

- **選択は存在を主張しない。** 源は箱の list 1 本だけで、list に無いものは表示されない
- **ダウンロード一覧の導出には使わない。** 一覧は箱の機械 list のままで、選択はその上の注記に留まる —
  研究のダウンロードの表と研究の箱の画面は、ファイルごとにそれを選んでいる dataset の ID を列に添える
  (逆引き)。箱の画面が読むのは公開されている dataset の選択で、公開ページと同じことを言う。draft が
  選んだだけのものはまだ出ない
...
まとめて行う道は公開前の確認の画面の「まとめて公開」が既に持っている (dataset が選んでいる未公開のファイルを
列挙し、その場で公開に倒す — [publishing.md](publishing.md))。research の箱の行に立つのは 5 つ —
ダウンロード・アドレスのコピー・公開 / 公開停止・名前の変更・削除。名前の隣の列はそのファイルを選んでいる
公開中の dataset の ID (上の「一覧と選択」) で、それぞれの公開ページを新しいタブで開く — 公開停止や削除の
前に、どの dataset のページから消えるかが読める。
...
**dataset がどのファイルを選ぶかは、dataset の編集画面の「ファイル」の節で決める。** 節が言うのは
紐づけた件数だけで、選んだものの一覧は隣の公開ページのペインが見せる。選ぶのは「ファイルの紐づけ」が開く
面 (`components/file-selection.tsx`) — 研究の箱の画面と同じ列 (ファイル名・サイズ・更新日・状態) の表の
先頭に checkbox を置き、紐づけ済みのものに印が付いた状態で開く。表の上の窓はファイル名で絞り、頭の
checkbox は窓が見せている行をまとめて選ぶ / 外す。**checkbox を変えてもフォームは変わらず、面の足元の
「紐づけ」で初めて入る** — 行を上から見比べている途中で、フォームと公開ページのペインが 1 つ押すたびに
描き直されないため。「キャンセル」で閉じれば選択は開く前のまま。**節には研究の箱の画面への道
(「ファイル一覧」) を新しいタブで置く** — まだ上げていないファイルはそこで上げてから選ぶ。未保存の
フォームを離れさせない。**選択は dataset の記述の一部なので、保存も照合も他の値と同じ**エントリ 1 行の
単位になる。**draft の段階では公開 bucket が空**なので、選ぶ相手は公開と非公開を合成した
admin 側の list。

**外部アーカイブの dataset には選ぶ節を出さない。** ファイル選択を伴う保存も、フォームが提供しない
