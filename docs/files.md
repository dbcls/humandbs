...
**research の箱では、公開状態の切り替えと削除を複数選んでまとめて行う。** 切り替えは job なので画面は
待たず、実行待ちと実行中を list の上に出す。**公開実績のあるファイルの削除には確認を挟む** — 外部から
直接参照されている実績があり、参照関係を持たない以上その検査ができない。

**`common/` の箱は upload する場所と表だけで、操作は行が持つ。** 切り替えが無いので選んでまとめて行う
ことが 1 つも無く、選ぶ列と表の外のボタンは「押したときに何に効くか」を行から離すだけになる。slug を
変えるのも消すのも、その行の中から始める。**upload はファイルを落としても選んでもよい** — フォルダは
受けず、落とされたら理由を言う (箱は flat な名前しか受け付けないため)。
