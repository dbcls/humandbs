...
import { inBoxOrder } from "./selection"
...
describe("inBoxOrder", () => {
  it("writes every name once, in name order", () => {
    fc.assert(fc.property(names, (selected) => {
      const written = inBoxOrder(selected)
  ...
  it("writes the same selection chosen in any order the same way", () => {
    fc.assert(fc.property(names, fc.nat(), (selected, seed) => {
      ...
      expect(inBoxOrder(turned)).toEqual(inBoxOrder(selected))
    }))
  })

  it("leaves a selection already written alone", () => {
    fc.assert(fc.property(names, (selected) => {
      expect(inBoxOrder(inBoxOrder(selected))).toEqual(inBoxOrder(selected))
    }))
  })

  it("orders by code point, the way the box lists, not by locale", () => {
    expect(inBoxOrder(["b.txt", "B.txt", "a.txt", "_x.txt"])).toEqual(["B.txt", "_x.txt", "a.txt", "b.txt"])
  })
})
