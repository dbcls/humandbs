...
const ja = {
  ...
  admin: {
    ...
    editor: {
      ...
      memo: "メモ",
      memoHint: "この下書きについて管理者どうしで残しておきます。公開ページにもプレビューにも出ません。",
      ...
      listingProvidersFrom: (names: string) =>
        names === ""
          ? "空のあいだは、提供者情報の研究代表者がそのまま一覧に出ます。"
          : `空のあいだは、提供者情報の研究代表者がそのまま一覧に出ます（いまは「${names}」）。`,
    ...
    templates: {
      ...
      takeProviderNote: "いまの提供者は消さず、申請の研究代表者を後ろに足します。",
      apply: "取り込み",
    ...
    catalog: {
      heading: "解析手法の表",
      backToList: "解析手法の表へ",
      note: "解析手法の表に用いる key を設定する。\"自由文\" 以外の key の追加などは開発者に依頼する。",
      code: "コード",
      labelJa: "日本語ラベル",
      labelEn: "英語ラベル",
      sortKeys: {
      ...
      active: "有効",
      inactive: "無効",
      states: { active: "有効", inactive: "無効" },
      offerTerm: "この値を選べるようにする",
      merge: "統合",
      mergeStart: "統合する",
      mergeChoosing: (label: string) => `${label} の統合先を選んでいます`,
      mergeChoosingNote: "残したい語の行で「ここへ統合」を押してください。",
      mergeInto: "ここへ統合",
      mergeCancel: "やめる",
      mergeTitle: (from: string, into: string) => `${from} を ${into} に統合`,
      mergeWarning:
        "この語を指している値は、すべて統合先を指すようになります。元の語は消えます。元に戻せません。",
      mergeConfirm: "統合",
      termsNote: "この項目の値に選べる語。無効にすると、これから書くデータセットでは選べなくなる。",
      usage: "使用",
      used: (count: number) => `${count} 件`,
      usedSearch: (count: number) => `${count} 件を検索結果で出す`,
      unused: "未使用",
      find: "コードかラベルで探す",
    ...
    contents: {
      ...
      slug: "slug",
      slugHint: "読者が開くアドレスになります。英小文字・数字・ハイフン・スラッシュで書きます。",
      find: "記事を探す",
      ...
      unpublish: "公開停止",
      removeLocale: "削除",
      removeLocaleTitle: (language: string) => `${language} の削除`,
      removeLocaleWarning: "この言語の本文が消えます。元に戻せません。",
      removeLocaleConfirm: "削除",
      removeLastDocumentWarning: "これが最後の言語です。消すと記事そのものが無くなり、slug も"
        + "解放されます。元に戻せません。",
      remove: "削除",
      ...
      cut: "バージョンへの切り出し",
      cutNote: (slug: string) =>
        `この記事をバージョン管理にします。いまの本文は ${slug}/version/{番号} に移り、`
        + `${slug} は最新版を指すアドレスになります。`,
      ...
      news: {
        ...
        untitled: "（題目未入力）",
        scheduled: "予約",
        scheduledNote: "この日時が来るまで、公開側には出ません。",
        datings: { dated: "あり", undated: "なし" },
        add: "お知らせの作成",
        removeLastWarning: "これが最後の言語です。消すとお知らせそのものが無くなります。"
          + "元に戻せません。",
        backToList: "お知らせ一覧へ",
      },
      files: {
