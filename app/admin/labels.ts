 ...
 * the administrator requests the next number and cannot choose the spelling, so
...
export function nhaId(number: number): string {
  if (!Number.isInteger(number) || number < 1 || number >= 10 ** NHA_WIDTH) {
    throw new RangeError(`no NHA id has ${number}`)
  }
  return `NHA${String(number).padStart(NHA_WIDTH, "0")}`
}

/** The number an id has, or null for any other spelling. */
 ...
 * the label's public prefix holds a file, and whether a switch of one of the
 * research's files has not finished. **They are told apart by what the reader
 * does next**, which is not the same for each:
 *
 * - `holds-files` — the primary's prefix holds files. Another label is made
 *   primary, and that moves them.
 * - `moving` — a retired label's prefix still holds files while a switch runs:
 *   the move making another label primary queued. Waiting is all it takes.
 * - `left-behind` — a retired label's prefix holds files and nothing is moving
 *   them. Only making a label primary queues a move, and it moves the prefix of
