...
describe("the dataset editing form", () => {
  it("shows only the items the dataset has, and offers the rest to be added", () => {
  ...
  it("keeps a vocabulary item's two state toggles after the search box, not ahead of the chosen values", () => {
    const html = render(view({
      ...emptyDatasetContent(),
      values: [{ keyId: VOCAB_KEY, value: { kind: "vocabulary", termIds: filled(["term-open"]) } }],
    }))
    ...
    const searchAt = html.indexOf("選択肢を探す")
    const toggleAt = html.indexOf("aria-label=\"未確定\"")
  ...
  it("puts every kind of value's own delete beside its own name, one per value", () => {
    ...
    // `IconButton` shows its name twice — `aria-label` and `title` — so one
  ...
  it("shows a number as it was typed, beside the unit it was typed in", () => {
    ...
    // The unit is shown in the site's own select: named on the closed box, lit in the list.
  ...
  it("marks the upper-end box wrong when it is typed below the lower end", () => {
    ...
    const highInput = /<input[^>]*value="0\.9"[^>]*>/.exec(html)?.[0] ?? ""
    expect(highInput).toContain("aria-invalid=\"true\"")
    expect(highInput).toMatch(/aria-describedby="[^"]+"/)
    expect(html).toContain("上限は下限より小さくできない")
    // The lower-end box itself is not the one marked wrong.
    const lowInput = /<input[^>]*value="1\.3"[^>]*>/.exec(html)?.[0] ?? ""
  ...
  it("shows a disease as the name somebody wrote and the code it is filed under", () => {
    ...
    // The classification's own heading is shown beside the name rather than
  ...
  it("puts an experiment's items under the experiment rather than the dataset", () => {
    ...
    // of that level is the one it already has, so nothing is offered here.
...
describe("what the form has but does not show", () => {
  it("passes the file selection into the form", () => {
...
describe("the header", () => {
  it("names the screen \"データセットの編集\", the dataset's own label beside it, and the back link to the list", () => {
    const html = render(view())
...
describe("the toolbar", () => {
  ...
  it("does not offer the import from the header", () => {
    expect(render(view())).not.toContain("/import\"")
  })
})
...
describe("which field of the form a press on the page focuses", () => {
  /**
   * A press on the page goes to the field the form marks with the same place
   * (`form.tsx` の `focusField`). A kind of field that marks nothing sends it to the
...
describe("the header of the dataset's form", () => {
  it("has the id and the dates, and shows the archive's dates are read, not written", () => {
    const html = render(view(emptyDatasetContent(), false))
    ...
    expect(head).toContain("外部アーカイブでその accession に登録されている日付を表示する。")
  ...
  it("keeps the id's box and its buttons at the row's height, among the facts of the line", () => {
...
describe("the dataset's own fields", () => {
  ...
  it("heads each field with its own name, under no heading that identifies them in general", () => {
  ...
  it("always has the type of data and the access type, even when the dataset has neither, and offers neither to be removed or added", () => {
...
describe("the link to a field's choices", () => {
  const key = (over: Partial<EditableCatalog["keys"][number]>) => ({ ...catalog.keys[0], ...over }) as EditableCatalog["keys"][number]
  const choicesLink = (over: Partial<EditableCatalog["keys"][number]>): string => {
    const Stub = createRoutesStub([{ path: "/*", Component: () => <ChoicesLink catalogKey={key(over)} locale="ja" /> }])
    return renderToStaticMarkup(<Stub initialEntries={["/admin/research/x/draft/y/dataset/z"]} />)
  }

  it("leads an experiment's vocabulary to its 「選べる値」, in a new tab so the form's unsaved work stays", () => {
    const html = choicesLink({ scope: "experiment", valueType: "vocabulary", code: "platform" })
  ...
  it("does the same for an experiment's disease", () => {
    expect(choicesLink({ scope: "experiment", valueType: "disease", code: "disease" })).toContain("/admin/experiment-fields/disease")
  })

  it("draws nothing for a dataset's own closed list, or a field that chooses from nothing", () => {
    expect(choicesLink({ scope: "dataset", valueType: "vocabulary", code: "access-criteria" })).not.toContain("<a")
    expect(choicesLink({ scope: "experiment", valueType: "text", code: "coverage" })).not.toContain("<a")
    expect(choicesLink({ scope: "experiment", valueType: "number", code: "read-length" })).not.toContain("<a")
  })
})

describe("the parts of an experiment's card", () => {
  it("keeps the experiment, its items and the way to add one under three headings, in that order", () => {
...
describe("adding an item", () => {
  it("waits for the 追加 button, which cannot be pressed until an item is chosen, and has no indicator of its own before the box", () => {
    const html = render(view(described()))
    ...
    // The box is shown alone: no glyph before it.
