...
describe("a select", () => {
  ...
  it("is drawn by the site, and has the choice in force in a hidden field", () => {
...
describe("a box that has to be filled", () => {
  it("is shown with the indicator hard against its name, with the word for anyone not looking", () => {
  ...
  it("keeps the indicator out of the badge that identifies its dialect", () => {
...
describe("a submit that cannot be pressed", () => {
  it("shows why over itself when handed a sentence", () => {
...
describe("a body's box", () => {
  ...
  it("stops at thirty lines and keeps a floor, rather than taking the room left", () => {
    const [, container] = /<\/textarea><div class="([^"]*)"/.exec(html) ?? []
    expect(container).toBeDefined()
    expect(container).toContain("max-h-[calc(30*1.3125rem+0.75rem+2px)]")
    expect(container).toContain("min-h-96")
    expect(container).not.toContain("flex-1")
  })
...
describe("a box the caret was brought to", () => {
  it("is shown with the warning background only while highlighted, and keeps the one ring colour", () => {
    const html = render(<TextArea label="研究題目" name="title" value="" />)
    expect(html).toContain("data-highlighted:bg-warning-surface")
    expect(html).not.toMatch(/data-highlighted:outline-/)
  })
})
...
describe("the news beside a save", () => {
  ...
  it("holds the room of every word it can show, out of sight and out of the reading order, while indicating nothing", () => {
  ...
  it("draws the word being said in the same cell, so what is shown beside it keeps its place", () => {
  ...
  it("shows a quiet word in the muted ink", () => {
...
describe("focusTargetPath", () => {
  ...
  it("takes the place itself when the form marks it", () => {
    fc.assert(fc.property(path, (at) => {
      expect(focusTargetPath(at, () => true)).toBe(at)
    }))
  })

  it("takes the nearest marked place the path runs through, and never one it does not", () => {
    fc.assert(fc.property(path, fc.array(path, { maxLength: 6 }), (at, annotatedPaths) => {
      const marked = new Set(annotatedPaths)
      const found = focusTargetPath(at, (candidate) => marked.has(candidate))
  ...
  it("focuses the element's row from a cell of a list's table", () => {
    const marked = new Set(["grants.g1", "title"])
    expect(focusTargetPath("grants.g1.title", (candidate) => marked.has(candidate))).toBe("grants.g1")
    expect(focusTargetPath("grants.g2.title", (candidate) => marked.has(candidate))).toBeNull()
    expect(focusTargetPath("title", (candidate) => marked.has(candidate))).toBe("title")
  })
})
...
describe("the answer to what was sent (Answer)", () => {
  ...
  it("shows what went through in one box that shows it is done", () => {
  ...
  it("shows a refusal as a failure", () => {
  ...
  it("raises nothing for an answer the screen does not apply to, nor for none", () => {
