...
describe("converting a number to the unit its key stores", () => {
  it("scales within a group of units", () => {
    ...
    expect(convert(1000, "MB", "GB")).toBe(1)
    expect(convert(150, "bp", "bp")).toBe(150)
    expect(convert(1.5, "kbp", "bp")).toBe(1500)
  })

  /** Neither group is a property of the data — both step by 1000 (SI), the portal's policy. */
  it("steps a data volume and a sequence length by 1000 alike", () => {
    expect(convert(1, "TB", "GB")).toBe(1000)
