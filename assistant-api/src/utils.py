...
def ensure_runtime_directories() -> None:
    for directory in [WORK_DIR, UPLOADS_DIR, RESULTS_DIR, LOGS_DIR, _PLAYWRIGHT_CACHE_DIR]:
...
def _load_icd10_descriptions() -> dict[str, str]:
    if not ICD10_MAPPING_PATH.exists():
        logger.warning("ICD-10 mapping file not found: %s", ICD10_MAPPING_PATH)
        return {}

    try:
        with open(ICD10_MAPPING_PATH, encoding="utf-8") as f:
            data = json.load(f)
        if isinstance(data, dict):
            return data
        logger.warning("ICD-10 mapping file has invalid format: %s", ICD10_MAPPING_PATH)
        return {}
    except Exception:
        logger.exception("Failed to load ICD-10 mapping file: %s", ICD10_MAPPING_PATH)
        return {}
...
def get_task_result_path(task_id: str) -> Path:
    return RESULTS_DIR / f"{task_id}.yml"


def get_task_error_path(task_id: str, label: str | None = None) -> Path:
    suffix = f"_{label}" if label else ""
    return RESULTS_DIR / f"{task_id}{suffix}_error.txt"


def get_all_task_result_paths() -> list[Path]:
    return sorted(RESULTS_DIR.glob("*.yml"), reverse=True)


def get_ethics_file_path(application_file_name: str) -> str | None:
    application_name = Path(application_file_name).stem
    return str(UPLOADS_DIR / f"{application_name}_ethics.pdf")


def get_research_plan_path(application_file_name: str) -> str | None:
    application_name = Path(application_file_name).stem
    return str(UPLOADS_DIR / f"{application_name}_research_plan.pdf")
...
def get_task_logger(task_id: str | None) -> logging.Logger:
    ...
    # Create file handler
    log_file = LOGS_DIR / f"{task_id or 'default'}.log"
...
def get_icd10_description(icd10_code: str) -> str:
    ...
    icd10_descriptions = _load_icd10_descriptions()
...
# Ensure runtime directories are ready during app startup.
ensure_runtime_directories()
