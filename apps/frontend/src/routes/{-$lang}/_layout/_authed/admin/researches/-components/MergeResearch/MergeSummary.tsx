...
import type { MergeFieldDescriptor } from "../utils/computeMergeFields";

export function MergeSummary({ overwritten }: { overwritten: MergeFieldDescriptor[] }) {
  const t = useTranslations("MergeWizard");

  if (overwritten.length === 0) {
    return <span className="text-gray-400 text-xs">{t("no-changes")}</span>;
  }

  return (
    <div className="flex min-w-0 items-baseline gap-1.5 text-xs">
      <span className="shrink-0 text-gray-500">{t("will-overwrite")}:</span>
