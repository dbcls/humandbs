...
function RouteComponent() {
  const [mode, setMode] = useState<"create" | "list" | (string & {})>();
  ...
  return (
    ...
    >
      {mode === "create" ? (
        <CreateResearchPage onBack={() => setMode("list")} />
      ) : (
        <>
...
function CreateResearchPage({ onBack }: { onBack: () => void }) {
  ...
  return (
    <section className="absolute inset-0 flex flex-col gap-5 overflow-auto">
      <Button variant={"ghost"} onClick={onBack}>
        <ChevronLeft className="inline size-5" /> To Researches list
      </Button>
      <form.AppForm>
