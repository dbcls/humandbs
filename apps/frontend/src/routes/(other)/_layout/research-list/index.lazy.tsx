...
function Caption() {
  ...
  return <div className="flex items-center justify-between">
    ...
    <div className="flex gap-4">
      ...
      <Input type="text" placeholder="検索" beforeIcon={<Search size={22} />} />
...
const data: ResearchData[] = [
  {
    ...
    "datasets": ["JGAS000770"],
    "title": "Progestin-primed ovarian stimulation (PPOS) 法を用いた調節卵巣刺激でプログステロン製剤が卵胞発育・卵子の成熟過程に及ぼす影響に関する検討",
  ...
  },
  {
    ...
    "datasets": ["JGAS000765"],
    "title": "胆道閉鎖症およびアラジール症候群由来iPS細胞を用いた胆管発生およびその障害メカニズムの解明",
  ...
  },
  {
    ...
    "datasets": ["JGAS000755"],
    "title": "日本人高齢者多い疾患のゲノム解析および臨床ゲノム情報ストレージの整備",
  ...
  },
  {
    ...
    "datasets": ["JGAS000740"],
    "title": "消化管癌・消化管腫瘍の組織検査による癌発育・進展メカニズムに関する研究",
  ...
  },
  {
    ...
    "datasets": ["JGAS000659", "JGAS000660", "JGAS000667", "E-GEAD-665", "E-GEAD-667"],
...
const columns = [
  columnHelper.accessor("researchId", {
    ...
    header: (ctx) => {
      ...
      const sortIcon = sortDirection === "desc" ? <ChevronDown size={18} /> : sortDirection === "asc" ? <ChevronUp size={18} /> : <ChevronsUpDown size={18} />

      return <div className=" flex items-center whitespace-nowrap"> <div>Research ID </div><Button onClick={ctx.column.getToggleSortingHandler()} variant={"plain"} className="p-0">{sortIcon}</Button></div>
  ...
  columnHelper.accessor("participants", {
    ...
    cell: ctx => ctx.getValue().map(participant => <div>
      <ul>
        {participant.content.map(content => <li key={content}>{content}</li>)}
      </ul>
      {participant.type && <p className=" text-secondary text-sm">{participant.type}</p>}
    </div>),
  }),
]
    ...
    <table className={cn("w-full align-top", className)}>
      <thead className=" text-white">
        {table.getHeaderGroups().map(headerGroup => (
          <tr key={headerGroup.id} className=" bg-linear-to-r from-secondary-light to-secondary-lighter rounded text-left ">
            {headerGroup.headers.map(header => (
              <th key={header.id} className={"p-2 first-of-type:rounded-l last-of-type:rounded-r"}>
        ...
        {table.getRowModel().rows.map(row => (
          <tr key={row.id}>
            {row.getVisibleCells().map(cell => (
              <td key={cell.id} className=" border-foreground-light/50 border-b-2 p-2 align-top">
