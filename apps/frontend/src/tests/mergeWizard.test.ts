...
describe("computeMergeFields – bilingual sub-field (summary.aims.ja)", () => {
  test("can-fill: current empty, incoming has value", () => {
    const fields = computeMergeFields(
      makeResearch({ summary: { aims: { ja: null, en: null }, methods: { ja: null, en: null }, targets: { ja: null, en: null }, url: { ja: [], en: [] } } }),
      makeResearch({ summary: { aims: { ja: { text: "目的" }, en: null }, methods: { ja: null, en: null }, targets: { ja: null, en: null }, url: { ja: [], en: [] } } }),
    );
  ...
  test("conflict: both non-empty and different", () => {
    const fields = computeMergeFields(
      makeResearch({ summary: { aims: { ja: { text: "旧" }, en: null }, methods: { ja: null, en: null }, targets: { ja: null, en: null }, url: { ja: [], en: [] } } }),
      makeResearch({ summary: { aims: { ja: { text: "新" }, en: null }, methods: { ja: null, en: null }, targets: { ja: null, en: null }, url: { ja: [], en: [] } } }),
    );
  ...
  test("same: both values equal", () => {
    const fields = computeMergeFields(
      makeResearch({ summary: { aims: { ja: { text: "同じ" }, en: null }, methods: { ja: null, en: null }, targets: { ja: null, en: null }, url: { ja: [], en: [] } } }),
      makeResearch({ summary: { aims: { ja: { text: "同じ" }, en: null }, methods: { ja: null, en: null }, targets: { ja: null, en: null }, url: { ja: [], en: [] } } }),
    );
...
describe("applyMergeDecisions – scalar field (title.ja)", () => {
  ...
  test("custom → custom value", () => {
    const result = applyMergeDecisions(fields, { "title.ja": "custom" }, { "title.ja": "カスタム" });
...
describe("applyMergeDecisions – array field (dataProvider)", () => {
  ...
  test("custom → custom array", () => {
    ...
    const result = applyMergeDecisions(fields, { dataProvider: "custom" }, { dataProvider: customArr });
...
describe("applyMergeDecisions – summary sub-field preserves {text} shape", () => {
  ...
  test("accepted aims.ja does not affect methods.ja", () => {
    ...
    const incomingWithMethods = makeResearch({ summary: summary({ text: "新目的" }, { text: "新方法" }) });
...
describe("applyMergeDecisions – independent decisions on sibling fields", () => {
  ...
  test("accepted title.en does not overwrite rejected title.ja", () => {
    const result = applyMergeDecisions(fields, { "title.ja": "rejected", "title.en": "accepted" }, {});
