...
export const $$parseSession = createServerOnlyFn((raw: string) => {
  try {
...
export const $$stringifySession = createServerOnlyFn((session: Session) => {
  return JSON.stringify(session);
});

export const $$getSession = createServerOnlyFn((): Session | null => {
...
export const $$verifyAccessToken = createServerOnlyFn(
  async (token: string, clockToleranceSec = 60) => {
    try {
 ...
 */
export const $$createSessionCookie = createServerOnlyFn((session: Session) => {
  return serialize(
    SESSION_COOKIE_NAME,
