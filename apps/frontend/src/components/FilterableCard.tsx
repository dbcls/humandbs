    ...
    <Card
      ...
      caption={
        <div className="relative">
          ...
          <div
            className={cn(
              "absolute top-[100%] right-0 z-50 mt-2 flex w-filter-panel flex-col drop-shadow-xl",
              "origin-top-right transition-all duration-300 ease-out",
              "max-h-[calc(100vh-8rem)]",
              {
                "pointer-events-none invisible scale-95 opacity-0": !panelOpen,
                "visible scale-100 opacity-100": panelOpen,
              },
            )}
          >
            {/* The arrow (caret) pointing up */}
            <div
              className="absolute bottom-[calc(100%-1px)] z-10 h-[8px] w-[16px] transition-all"
            ...
            <div className="relative z-0 flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white">
              <Suspense>
                {renderPanel({
                  onClose: () => {
                    setPanelOpen(false);
