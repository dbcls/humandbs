...
const cardWithColorCaptionVariants = cva(" bg-linear-to-r rounded-t-md text-white", {
  variants: {
    variant: {
...
interface CardWithCaptionProps extends CardProps, VariantProps<typeof cardWithColorCaptionVariants> { }

function CardWithCaption({ children, className, caption, size, variant, ...rest }: CardWithCaptionProps) {

  return (
    <div className={cn("rounded-md bg-white h-fit", className)} {...rest}>
      {caption ?
        <div className={cn(cardWithColorCaptionVariants({ size, variant, className }))}>
          {caption}
        </div> : null
      }
