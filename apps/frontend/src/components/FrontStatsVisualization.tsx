...
type SelectionLike = {
  attr: (...args: unknown[]) => SelectionLike;
  datum: (...args: unknown[]) => any;
  append: (...args: unknown[]) => SelectionLike;
  call: (...args: unknown[]) => SelectionLike;
  on: (...args: unknown[]) => SelectionLike;
  style: (...args: unknown[]) => SelectionLike;
  html: (...args: unknown[]) => SelectionLike;
  text: (...args: unknown[]) => SelectionLike;
};
...
function normalizeStatsResponse(payload: unknown): NormalizedStats | null {
  ...
  for (const [facet, values] of Object.entries(stats.facets ?? {})) {
    ...
    systems.push({
      facet,
      total: d3.sum(satellites, (d: StatsSatellite) => d.total),
      satellites,
    });
  }
    ...
    async function load() {
      try {
        ...
        if (mounted) {
          ...
          setState({
            loading: false,
            error: `Could not load /api/stats (${message}).`,
            stats: null,
          });
        }
      }
...
function PlanetChart({ stats }: { stats: NormalizedStats }) {
  const svgRef = useRef<SVGSVGElement | null>(null);
  const tooltipRef = useRef<HTMLDivElement | null>(null);

  const systems = useMemo(() => stats.systems.slice(0, 8), [stats.systems]);
  const satellites = useMemo(
    () => systems.flatMap((system) => system.satellites.slice(0, 24)),
    [systems],
  );
  ...
  useEffect(() => {
    ...
    const satExtent = d3.extent(satellites, (d: StatsSatellite) => d.total);
    ...
    const hubAnchors: SelectionLike[] = [];
    const satelliteSystems: {
      entries: {
        circle: SelectionLike;
        label: SelectionLike | null;
        angle: number;
        speed: number;
        orbitRadius: number;
        radiusPx: number;
    ...
    systems.forEach((system, i) => {
      ...
      const hubRadius = Math.max(
        ...
        Math.sqrt(
          d3.sum(
            satLayout,
            (d: { radiusPx: number }) => d.radiusPx * d.radiusPx,
          ),
        ),
      );
      ...
      const maxSatRadius =
        satLayout.length > 0
          ? (d3.max(satLayout, (d: { radiusPx: number }) => d.radiusPx) ?? 0)
          : 0;
      ...
      hub
        ...
        .on("mousemove", (event: MouseEvent) => {
          const researchTotal = d3.sum(
            localSatellites,
            (d: StatsSatellite) => d.research,
          );
          const datasetTotalFacet = d3.sum(
            localSatellites,
            (d: StatsSatellite) => d.dataset,
          );
          const satellitesRows = localSatellites
            .map(
              (d: StatsSatellite) =>
                `<div class="front-stats__tooltip-row"><span>${d.value}</span><strong>${d.total}</strong></div>`,
            )
            .join("");
          tooltip
            .style("opacity", 1)
            ...
            .style("top", `${event.offsetY + 18}px`).html(`
              <div class="front-stats__tooltip-title">${system.facet}</div>
              <div class="front-stats__tooltip-row"><span>Research</span><strong>${researchTotal}</strong></div>
              <div class="front-stats__tooltip-row"><span>Dataset</span><strong>${datasetTotalFacet}</strong></div>
              <div class="front-stats__tooltip-row"><span>Total</span><strong>${system.total}</strong></div>
              <div class="front-stats__tooltip-divider"></div>
              ${satellitesRows}
            `);
        })
        .on("mouseleave", () => {
      ...
      satLayout.forEach(
        ({ sat, satIdx, seedAngle, seedRadius, seedSpeed, radiusPx }) => {
          ...
          const circle = satellitesLayer
            ...
            .attr(
              "fill",
              `url(#front-stats-planet-grad-${(paletteIndex + satIdx + 2) % palettes.length})`,
            )
            .attr("stroke", "#ecf0f1")
            .attr("stroke-width", 0.9);

          let label: SelectionLike | null = null;
          if (sat.total >= minSatTotal) {
            label = satellitesLayer
              .append("text")
              .attr("class", "front-stats__sat-label")
              .attr("x", sx)
    ...
    const spin = d3.timer((elapsedMs: number) => {
      if (!running) return;
  ...
  return (
    <div className="front-stats__chart-wrap">
      <svg
        ref={svgRef}
        width="100%"
      ...
      />
      <div ref={tooltipRef} className="front-stats__tooltip" />
    </div>
  );
}
