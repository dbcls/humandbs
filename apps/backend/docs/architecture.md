...
Research のレスポンスは「詳細 (`GET /research/{humId}`, `GET /research/{humId}/versions/{v}`)」と「一覧／検索 (`GET /research`, `POST /research/search`, `POST /research/{humId}/dataset/search` の Research 側)」で別シェイプを返す。
