...
間違っていても検索でヒットしない程度の影響なので、随時修正できる。
...
`tissues`, `assayType` などの一部フィールドは、LLM 抽出後に機械的な処理（[facet-normalize](crawler-pipeline.md#9-facet-normalize)）で表記を統一する。
