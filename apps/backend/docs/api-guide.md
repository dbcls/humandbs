...
| `parentJgaStudyId` | `string \| null` | detail / version detail / batch | DDBJ Search API (`/search/resources/jga-dataset/_doc/{JGAD_ID}`) を live fetch |
...
DDBJ Search API (`https://ddbj.nig.ac.jp/search/resources/jga-dataset/_doc/{JGAD_ID}`) の `_source.dbXrefs` から `type === "jga-study"` の先頭 `identifier` を採る。

Base URL は `HUMANDBS_DDBJ_SEARCH_RESOURCES_BASE_URL` 環境変数で上書き可能。timeout は既定 3000ms、`HUMANDBS_DDBJ_SEARCH_RESOURCES_TIMEOUT_MS` で上書き可能。
