...
- **Dataset** (`/dataset`) — Research に従属するデータセット単位（例: `JGAD000001`）。1 つの Research に対し 1:N。detail / version detail / batch のレスポンスには JGAD の親 JGAS accession を `parentJgaStudyId` として動的に付与する（§10 参照）
...
- admin-only operation: `createResearch`, `deleteResearch`, `approveResearch`, `rejectResearch`, `unpublishResearch`, `getResearchOwners`, `deleteDataset`, JGA Shinsei 全 endpoint
...
`POST /research/{humId}/dataset/new` の `datasetId` は optional。省略時はサーバ側で `DRAFT-{humId}-{uuid}` を自動採番するため衝突しない。明示指定した場合は **system 全体で unique** な扱いで、既存 `datasetId`（別 Research 配下のものを含む）を指定すると `409 Conflict` を返す。これは、既存 datasetId で create すると旧 version が UI 上見えなくなる形の上書き、および別 Research からの datasetId 奪取（`getNextDatasetVersion` が新 version を採番し `linkDatasetToResearch` が呼出し先 Research の datasets 配列にその version を張る）を作成 endpoint で塞ぐため。
...
`title` / `summary` / `summaryShort` / `dataProvider` / `researchProject` / `grant` / `relatedPublication` は per-version snapshot を持つ ([data-model.md § Research と ResearchVersion のフィールド分担](data-model.md))。
...
- operationId: `updateResearch`, `updateDataset`
...
| レスポンス item | 詳細エンドポイントと同じシェイプ（dataset は `mergedSearchable` と `parentJgaStudyId` 付き）。楽観的ロック値（`_seq_no` / `_primary_term`）は **含めない**（一覧と同思想、編集時は詳細を再取得する） |

`parentJgaStudyId` は per-ID で DDBJ Search を live fetch するため、一部 ID の取得失敗が他 item に伝播しない（該当 item のみ `null`。§10 参照）。`distribution` は現状 batch item には含めない（詳細エンドポイントのみ）。
...
| **Dataset** (`JGAD000001`) | `JGAD` (1:1)。JGAD の親 JGAS accession は dataset レスポンスの `parentJgaStudyId` として surface する（§10） | DRA Submission (`DRA000xxx`) (1:1) |
...
## 10. Dataset レスポンスの動的生成フィールド

Dataset detail / version detail / batch エンドポイントのレスポンスには、ES に永続化されていない動的生成フィールドが 2 つ含まれる。いずれもレスポンス構築時に組み立て、失敗しても dataset 本体は正常に返す（該当フィールドのみ空 or `null`）。

| フィールド | 型 | 含まれる endpoint | 生成元 |
|---|---|---|---|
| `distribution` | `DistributionItem[]` | detail / version detail | ID パターン別に URL 構築 (`api/utils/distribution.ts`) |
| `parentJgaStudyId` | `string \| null` | detail / version detail / batch | DDBJ Search API dblink (`GET /search/api/dblink/jga-dataset/{JGAD_ID}`) を live fetch |

### Distribution（データダウンロードリンク）

Dataset detail / version detail のレスポンスに、公開データのダウンロードリンクを `distribution` フィールドとして含める。
...
### 親 JGAS accession（`parentJgaStudyId`）

JGAD dataset の親 JGAS accession を、detail / version detail / batch のレスポンスにトップレベル `parentJgaStudyId` フィールドとして含める。ES には保存せず、レスポンス構築時に DDBJ Search API を live fetch する。

#### 対象

`datasetId` が `JGAD` prefix のもののみ。`DRA` / `E-GEAD-` / `MTBKS` / `hum...` 等の非 JGAD dataset では DDBJ Search を叩かず、フィールド値は常に `null`。

#### 取得元

DDBJ Search API dblink (`https://ddbj.nig.ac.jp/search/api/dblink/jga-dataset/{JGAD_ID}`) の `dbXrefs` から `type === "jga-study"` の先頭 `identifier` を採る。既存の distribution 実装と同じ `fetchDblink` client を再利用する。

Base URL は `HUMANDBS_DDBJ_SEARCH_API_BASE_URL` 環境変数で上書き可能 (default `https://ddbj.nig.ac.jp/search/api`、`client.ts` と共通)。
...
親 JGAS が特定できない場合（対象外 ID / 404 / dbXrefs 不在 / DDBJ 障害 / timeout）は `null`。

#### キャッシュ

DDBJ Search の負荷軽減とレスポンス速度のため、`datasetId` → `parentJgaStudyId` の対応を in-memory の TTL キャッシュに保持する（既定 TTL = 1 時間、`CACHE_TTL.JGA_PARENT_STUDY`）。`null`（親 JGAS が特定できないケース）も同じ TTL でキャッシュする。DDBJ 呼び出しが例外を投げた場合はキャッシュしない（次リクエストで retry される）。

#### エラー時の挙動

DDBJ Search が network error / 5xx / timeout / JSON parse error のいずれでも `parentJgaStudyId` を `null` にし、dataset 本体は 200 で返す（`distribution` と同じ思想）。batch では per-ID で fetch するため、単一 ID の失敗が他 item に伝播しない。

### 関連 OpenAPI 参照

- operationId: `getDataset`, `getDatasetVersion`, `batchGetDatasets`
