...
- `dataProvider[0]`: J-DS の PI を `Person` に整形 (氏名は `last first` (ja) / `first last` (en) で連結、所属・国コードも含む)
- `relatedPublication`: `jds.publication` (free-text) を `[{ title: { ja: null, en: "..." } }]` で 1 件
- `relatedAccessions.jgad`: `jds.jgaIds[]` を `^JGAD\d+$` で filter したリスト。後段で `GET /templates/dataset/{externalId}` を叩く際の入力源
- `warnings[]`: 非致命的な注意事項を入れる将来用のフィールド。現状この Research ルートで warning を発生させる経路はないため、常に空配列が返る
...
J-DS の `restriction` / `icd10` / `collaborators[]` / `submitter` は雛形に直接マッピングしない (humandbs 側に対応スロットがないか、雛形の意味から外れるため admin が手で扱う)。
...
- 内部 traversal: `DRA submission → DRP (/dblink) → DRX 群 (/dblink) → 各 DRX について DRR / DRS / BioSample`
- 各 DRX については `/entries/sra-experiment/{drx}` と `/dblink/sra-experiment/{drx}` を 1 回ずつ取得し、`dbXrefs[]` を type で分けて DRR / DRS / BioSample の identifier を抜き出す (DRX 1 個あたりの dblink リクエストは 1 回)
- `criteria = "Unrestricted-access"`
- `typeOfData.en = submission.title or description`
- `experiments[]`: **1 DRX = 1 experiment row**。`header = { ja: DRX, en: DRX }`、`data` に library 情報 (`Library Strategy` / `Library Layout` / `Platform` / `Instrument Model` 等) + `Run Accessions` + `BioSample` + DRS の `Organism` + BioSample attributes (`harmonized_name` / `attribute_name` をキーにそのまま展開、snake_case 維持)
- `experiments[].searchable`: DRX から **機械抽出できる項目だけ** 埋める。`assayType` (= libraryStrategy)、`platforms` (= platform + instrumentModel の組)、`readType` (= libraryLayout の `PAIRED` / `SINGLE` → `paired-end` / `single-end`)。他フィールド (`subjectCount` / `diseases` / `tissues` / `healthStatus` 等) は不安定なので default (`null` / `[]`) のまま admin / 後段 LLM 抽出に委ねる
- DDBJ Search API への並列 fetch は同時 5 並列まで (常時 5 worker、終わったら順次次の DRX を補充するセマフォ式)
- 部分失敗 (DRX / DRS / BioSample 取得失敗) は `warnings[]` に `"DRX000003: fetch failed (...)"` または `"DRX000003 DRS DRS000123: fetch failed (...)"` の形で記録し、取れた分は返す
