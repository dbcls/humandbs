...
JGA 申請データ (DS: データ提供申請 / DU: データ利用申請) の read-only API。全エンドポイントに admin 認証が必要。

データは `jga-shinsei/json-data/` の変換済み JSON を ES にインデックスしたもの。レスポンススキーマはゆるい (`z.record(z.unknown())`) 定義。
