/**
 * Export misc pages to JSON
 *
 * Reads raw misc page data from Joomla DB output, cleans HTML,
...
/**
 * Build the original URL for a misc page
 */
...
/**
 * Process a single raw misc page into MiscPageContent
 */
 ...
 * Export misc pages to JSON
 */
const exportMiscPages = (): void => {
  const joomlaDir = getJoomlaDir()
  const inputPath = join(joomlaDir, "output", "misc-pages-raw.json")
  const outputPath = join(joomlaDir, "output", "misc-pages.json")

  console.log(`Reading raw misc pages from ${inputPath}`)

  const rawData = readJson<unknown>(inputPath)
  if (!rawData) {
    console.error(`File not found: ${inputPath}`)
    console.error("Run ./scripts/export-misc-json.sh first")
    process.exit(1)
  }
  ...
  if (!parseResult.success) {
    console.error("Invalid raw misc pages data:", parseResult.error)
    process.exit(1)
  }
  ...
  console.log(`Exported ${output.totalCount} misc pages to ${outputPath}`)
}
