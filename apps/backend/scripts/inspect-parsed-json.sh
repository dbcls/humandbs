#!/bin/bash
# Extract unique field values from detail-json files for debugging/analysis
# Usage: ./scripts/inspect-parsed-json.sh
...
OUT_DIR="$RESULTS_DIR/parsed-field-analysis"

if [ ! -d "$JSON_DIR" ]; then
  echo "Error: $JSON_DIR not found. Run 'bun run crawler:parse-html' first."
  exit 1
fi
...
echo "Extracting field values from parsed JSON..."
...
for lang in ja en; do
  ...
  # Molecular Data (id only, not data keys/values)
