...
import { parseHomeHtml, parseHomeTitles } from "@/crawler/home"
...
describe("home.ts", () => {
  describe("parseHomeHtml", () => {
    ...
    it("should return humIds in correct format", () => {
      const humIds = parseHomeHtml(homeHtml)
    ...
    it("should return empty array for HTML without table", () => {
      const humIds = parseHomeHtml("<html><body><p>No table</p></body></html>")
      expect(humIds).toEqual([])
    })
  })
  ...
  describe("parseHomeTitles", () => {
    ...
    it("should use last occurrence for duplicate humIds", () => {
      // hum0002 appears twice in fixture (v1 and v2)
      const mapping = parseHomeTitles(homeHtml)
      // The last one (v2) should be used
