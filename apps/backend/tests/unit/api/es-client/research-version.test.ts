...
describe("createResearchVersion (IT-VERSION-06)", () => {
  it("copies datasets from the latest version when datasets arg is undefined", async () => {
    ...
    // Opening a draft publishes nothing, so the root's derived dates stay put.
    expect(updateArgs.body.doc).not.toHaveProperty("dateModified")
    expect(updateArgs.body.doc).not.toHaveProperty("datePublished")
  })
