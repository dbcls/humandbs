...
**関連 unit テスト**: `tests/unit/api/middleware/request-id.test.ts` (未作成)
...
**関連 unit テスト**: `tests/unit/api/routes/stats.test.ts` (現状はルート登録のみ)
...
**関連 unit テスト**: `tests/unit/api/routes/stats.test.ts` (拡充予定)
...
**関連 unit テスト**: `tests/unit/api/middleware/auth.test.ts` (未作成)
...
- 各 item の `status` フィールドの値は `"published"` (value-based field control)、`uids` は `[]`、`draftVersion` は `null`

**回帰元**: `architecture.md § 公開条件` / `architecture.md § 値ベースの制御`
...
**関連 unit テスト**: `tests/unit/api/routes/facets.test.ts` (現状はルート登録のみ)
...
**関連 unit テスト**: `tests/unit/api/es-client/search.test.ts` (新規)
...
**関連 unit テスト**: `tests/unit/api/es-client/dataset.test.ts` (新規)
...
- 各 item の `status === "published"`、`uids === []`、`draftVersion === null` (value-based)
- `_seq_no` / `_primary_term` は含まれる
...
**関連 unit テスト**: `tests/unit/api/es-client/research-version.test.ts` (新規)
...
**関連 unit テスト**: `tests/unit/api/routes/research/versions.test.ts` (新規)
