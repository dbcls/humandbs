...
import { RATE_LIMIT_DELAY_MS, MIN_TITLE_LENGTH } from "@/crawler/config/urls"
...
const getCacheDir = (): string => {
  const dir = join(getExternalCacheDir(), "doi")
  ensureDir(dir)
  return dir
}
...
const searchCrossref = async (title: string, fromYear?: number): Promise<CrossrefWorkItem[]> => {
  ...
  try {
    ...
    logger.debug("Crossref search completed", { title: title.slice(0, 50), results: items.length })
    return items
  } catch (error) {
    logger.error("Failed to search Crossref", { title: title.slice(0, 50), error: getErrorMessage(error) })
    return []
  }
}
...
export const batchSearchDois = async (
  ...
  for (let i = 0; i < publications.length; i++) {
    ...
    if (isDoiUrl(pub.doi)) {
      logger.debug("DOI already present", { title: title.slice(0, 50), doi: pub.doi })
      result.set(title, {
    ...
    if (useCache && title in cacheEntry.results) {
      ...
      logger.debug("DOI cache hit", { title: title.slice(0, 50), found: cached !== null })
  ...
  writeJson(cachePath, cacheEntry)
...
export const loadDoiCache = (humId: string): DoiCacheEntry | null => {
  ...
  return readJson<DoiCacheEntry>(cachePath)
}
