...
export const DatasetFiltersSchema = z.object({
  ...
  criteria: z
    ...
    .optional()
    .describe(
      "Data access criteria: Controlled-access (Type I/II), Unrestricted-access. Array for OR logic.",
    ),
  subjectCountType: z
    .array(z.enum(["individual", "sample", "mixed"]))
    .optional()
    .describe("Subject count type: individual, sample, or mixed"),
  healthStatus: z
    .array(z.enum(["healthy", "affected", "mixed"]))
    .optional()
  ...
  diseaseIcd10: z
    ...
    .optional()
    .describe(
      "ICD-10 disease codes (prefix match, e.g., 'C34' matches 'C34.1')",
    ),
  tissues: z
    .array(z.string())
    .optional()
    .describe("Tissue types (facet selection)"),
  isTumor: IsTumorSchema.optional().describe(
    "Filter by tumor sample status: 'tumor', 'normal', or 'mixed'",
  ),
  cellLine: z
    .array(z.string())
    .optional()
  ...
  ageGroup: z
    .array(z.enum(["infant", "child", "adult", "elderly", "mixed"]))
    .optional()
  ...
  readType: z
    .array(z.enum(["single-end", "paired-end"]))
    .optional()
  ...
  policyId: z.array(z.string()).optional().describe("Data access policy IDs"),

  // Range filters
  releaseDate: RangeFilterSchema.optional().describe(
    "Dataset release date range (ISO 8601 format, e.g., {min: '2020-01-01', max: '2024-12-31'})",
  ),
  subjectCount: RangeFilterSchema.optional().describe(
    "Subject count range (e.g., {min: 100, max: 1000})",
  ),
  readLength: RangeFilterSchema.optional().describe(
    "Read length range in base pairs",
  ),
  sequencingDepth: RangeFilterSchema.optional().describe(
    "Sequencing depth range (x)",
  ),
  targetCoverage: RangeFilterSchema.optional().describe(
    "Target coverage range (%)",
  ),
  dataVolumeGb: RangeFilterSchema.optional().describe(
    "Data volume range in GB",
  ),
  variantSnv: RangeFilterSchema.optional().describe("SNV variant count range"),
  variantIndel: RangeFilterSchema.optional().describe(
    "Indel variant count range",
  ),
  variantCnv: RangeFilterSchema.optional().describe("CNV variant count range"),
...
export const ResearchSearchBodySchema = z.object({
  ...
  datasetFilters: DatasetFiltersSchema.optional().describe(
...
export const DatasetSearchBodySchema = z.object({
  ...
  // Dataset filters
  filters: DatasetFiltersSchema.optional().describe(
    "Filter Datasets by various attributes",
  ),
