...
export const SearchableExperimentFieldsSchema = z.object({
  ...
  ageGroup: z
    .enum(["infant", "child", "adult", "elderly", "mixed"])
    .nullable()
  ...
  libraryKits: z.array(z.string()).describe("Library preparation kits used"),

  // Platform (nested for vendor/model relationship)
  platforms: z.array(PlatformInfoSchema).describe("Sequencing platforms used"),
  readType: z
    .enum(["single-end", "paired-end"])
    .nullable()
    .describe("Read type: 'single-end' or 'paired-end'"),
  readLength: z.number().nullable().describe("Read length in base pairs"),
  ...
  // Target region
  targets: z.string().nullable().describe("Target regions or gene panels"),

  // Variant data
  variantCounts: VariantCountsSchema.nullable().describe(
    "Variant counts by type (SNV, indel, CNV, SV)",
  ),
  hasPhenotypeData: z
    .boolean()
    .nullable()
...
export const EsDatasetSchema = z.object({
  ...
  typeOfData: BilingualTextSchema.describe(
    "Bilingual description of the type of data in this dataset",
  ),
  experiments: z
    .array(EsExperimentSchema)
    .describe(
...
export const EsResearchSchema = z.object({
  ...
  url: BilingualTextSchema.describe(
    "URLs to the Research detail page (Japanese and English)",
  ),
  title: BilingualTextSchema.describe("Research title in Japanese and English"),
  summary: EsSummarySchema.describe(
    "Research summary including aims, methods, and targets",
  ),
  dataProvider: z
    .array(EsPersonSchema)
    .describe("Data providers (researchers providing the data)"),
  researchProject: z
    .array(EsResearchProjectSchema)
    .describe("Related research projects"),
  grant: z.array(EsGrantSchema).describe("Funding grants"),
  relatedPublication: z
    .array(EsPublicationSchema)
  ...
  versionIds: z
    .array(z.string())
    .describe(
      "List of version identifiers (e.g., ['hum0001.v1', 'hum0001.v2'])",
    ),
  latestVersion: z.string().describe("Latest version number (e.g., 'v2')"),
  datePublished: z
    .string()
  ...
  status: ResearchStatusSchema.describe(
    "Publication status: 'draft', 'review', 'published', or 'deleted'",
  ),
  uids: z
    .array(z.string())
...
export const EsResearchVersionSchema = z.object({
  ...
  version: z.string().describe("Version number (e.g., 'v1', 'v2')"),
  versionReleaseDate: z
    .string()
    .describe("ISO 8601 date when this version was released"),
  datasets: z
    .array(DatasetRefSchema)
