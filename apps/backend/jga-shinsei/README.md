...
ES への投入バッチは廃止済み。検索・フィルタ要件がないため、API リクエストごとに PostgreSQL を直接叩いてレスポンスを組み立てる。EAV → ネスト構造への変換は `transform.ts` の純粋関数を流用する。

`scripts/dump-all-data.sh` は SQL リファレンス / 手動デバッグ用途として残す (live API は経由しない)。

### ソースコード配置

| パス | 内容 |
|------|------|
| `src/crawler/processors/jga-shinsei/transform.ts` | EAV → API フレンドリー変換 (per-request で再利用) |
| `src/crawler/processors/jga-shinsei/reverse-transform.ts` | API フレンドリー → EAV 逆変換 (将来用、現在 API は read-only) |
...
以下のセクションは live API の経路には含まれない。dump スクリプトはデバッグ・分析用途、変換ロジックの解説は per-request transform の理解用。
...
| メソッド | パス | 認証 | 説明 |
|---------|------|------|------|
| GET | `/jga-shinsei/ds` | admin | DS 申請一覧 (page+limit、jdsId 昇順) |
| GET | `/jga-shinsei/ds/{jdsId}` | admin | DS 申請詳細 |
| GET | `/jga-shinsei/du` | admin | DU 申請一覧 (page+limit、jduId 昇順) |
