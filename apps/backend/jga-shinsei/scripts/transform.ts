 ...
 *
 * 入力:
 *   - json-data/component-keys.json (multiValue フラグ参照)
 ...
 * - getValues: 複数値を配列で取得
 */
export function createHelpers(components: Component[]): ComponentHelpers {
...
function buildHead(getValue: (key: string) => string | null): Head {
  return {
...
function buildPi(getValue: (key: string) => string | null): Pi {
  return {
...
function buildSubmitter(getValue: (key: string) => string | null): Submitter {
  return {
...
export function buildUseDatasets(
  getValues: (key: string) => string[],
): UseDataset[] {
...
export function transformDsApplication(
  ...
  return {
    ...
    control: buildControl(getValue, getValues),
...
export function transformDuApplication(
